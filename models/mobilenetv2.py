...
def conv2d(inplanes, out_planes, kernel_size=3, stride=1, padding=0, groups=1, bitw=32):
    """convolution with padding"""
    return qnn.QuantConv2d(inplanes, out_planes, kernel_size=kernel_size, stride=stride,
                     padding=padding, groups=groups, bias=False,
                     nbits=bitw, symmetric=qcfg['symmetric'],
                     centralize=qcfg['centralize'], do_mask=qcfg['do_mask'])


def relu6(inplace=False):
    """ReLU activation"""
    return qnn.QuantReLU6(inplace=False, nbits=qcfg['bita'])

'''
class ConvBNReLU(nn.Sequential):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, groups=1, bitw=32):
        padding = (kernel_size - 1) // 2 if kernel_size > 1 else 0
        super(ConvBNReLU, self).__init__(
            conv2d(in_planes, out_planes, kernel_size, stride, padding, groups=groups, bitw=bitw),
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        if expand_ratio != 1:
            # pw
            layers.append(ConvBNReLU(inp, hidden_dim, kernel_size=1, bitw=qcfg['bitw']))
        layers.extend([
            # dw
            ConvBNReLU(hidden_dim, hidden_dim, stride=stride, groups=hidden_dim, bitw=qcfg['bitw']),
            # pw-linear
            conv2d(hidden_dim, oup, 1, 1, bitw=qcfg['bitw']),
            nn.BatchNorm2d(oup),
        ])
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, image_size=224, num_classes=1000, width_mult=1.0):
        ...
        features.append(ConvBNReLU(input_channel, self.last_channel, kernel_size=1, bitw=qcfg['bitw']))
...
def mobilenetv2(data='cifar10', qnn=None, cfg=None):
    r"""
    Args:
