...
from .quantizer_cent import CQ


class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros',
                 nbits=32, symmetric=False, centralize=False, do_mask=False, **kwargs):
        ...
        if self.nbits != 32:
            if do_mask:
                self.quantizer = CQ(self.nbits, self.q_n, self.q_p, weight=self.weight)
            else:
...
class QuantLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, nbits=32, symmetric=False, centralize=False, do_mask=False, **kwargs):
